...
extern "C" int calculate_atomistic_mcsh(double** cell, double** cart, double** scale, int* pbc_bools, int center_atom_bool,
                                        int* atom_i, int natoms, int* cal_atoms, int cal_num,
            ...
            for (int dy=min_bin[1]; dy < max_bin[1]+1; ++dy) {
                for (int dz=min_bin[2]; dz < max_bin[2]+1; ++dz) {
                    ...
                    for (int j=0; j < natoms; ++j) {
                        ...
                        // same atom
                        if (!center_atom_bool && (!(cell_shift[0] || cell_shift[1] || cell_shift[2]) && (i == j)))
                            continue;
